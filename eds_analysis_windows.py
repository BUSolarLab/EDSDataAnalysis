...
def manual_sort_dates(df, eds_cols, window):
...
def manual_sort_time(df, eds_cols, window):
    # declare initial variables
    col_name = 'Time'
    counter = 0
    time_data = []
...
def manual_sort_labels(df, eds_cols, window):
    # declare initial variables
    col_name = 'EDS(#)'
    counter = 0
    new_col = {col_name:[]}
...
def manual_sort_data(df, name, eds_cols, window):
    # declare initial variables
    col_name = name
    counter = 0
    data = []
...
def get_avg_manual_data(loc, cols_list, window):
    # get the csv file
    df = pd.read_csv(loc)
    ...
    eds_cols = {}
    # sort the date
    eds_cols = manual_sort_dates(df, eds_cols, window)
    # sort the time
    eds_cols = manual_sort_time(df, eds_cols, window)
    # sort the pre/post, EDS number
    eds_cols = manual_sort_labels(df, eds_cols, window)
    # sort all the numerical data
    for x in cols_list:
        eds_cols = manual_sort_data(df, x, eds_cols, window)
...
def noon_read_data(df):
    ...
    return table
# sort the date column of the csv file
def noon_sort_dates(df,eds_cols, window):
    # declare initial variables
    col_name = 'Date'
    counter = 0
    date_string = ''
    ...
    return eds_cols
# sort the time column of the csv file
def noon_sort_time(df,eds_cols, window):
    # declare initial variables
    col_name = 'Time'
    counter = 0
    index_counter = 0
...
def noon_sort_labels(df,eds_cols, window):
    # declare initial variables
    col_name = 'PRE/POST'
    col_name2 = 'EDS/CTRL(#)'
    counter = 0
    ...
    return eds_cols
# sort the desired data from other columns from the csv file
def noon_sort_data(df,name, eds_cols, window):
    # declare initial variables
...
def get_avg_noon_data(loc,cols_list, window):
    # get csv file
    df = pd.read_csv(loc)
    ...
    eds_cols = {}
    # sort the date
    eds_cols = noon_sort_dates(df,eds_cols, window)
    # sort the time
    eds_cols = noon_sort_time(df,eds_cols, window)
    # sort the pre/post, EDS number
    eds_cols = noon_sort_labels(df,eds_cols, window)
    # sort all the numerical data
    for x in cols_list:
        eds_cols = noon_sort_data(df,x, eds_cols, window)
    # create new dataframe
    eds_df = pd.DataFrame(eds_cols)
    return eds_df
...
def testing_sort_dates(df,eds_cols, window):
    # declare initial variables
    col_name = 'Date'
    counter = 0
    date_string = ''
...
def testing_sort_time(df,eds_cols, window):
    # declare initial variables
    col_name = 'Time'
    counter = 0
    index_counter = 0
    ...
    return eds_cols
# sort the panel labels for the csv file
def testing_sort_labels(df,eds_cols, window):
    # declare initial variables
    col_name2 = 'EDS(#)'
    counter = 0
    new_col2= {col_name2:[]}
    ...
    return eds_cols
# sort the desired data from other columns from the csv file
def testing_sort_data(df,name, eds_cols, window):
    # declare initial variables
    col_name = name
    counter = 0
    index_counter = 0
...
def get_avg_testing_data(loc,cols_list, window):
    # get csv file
    df = pd.read_csv(loc)
    # check if window is 1 day
    if window == 1:
    ...
    eds_cols = {}
    # sort the date
    eds_cols = testing_sort_dates(df, eds_cols, window)
    # sort the time
    eds_cols = testing_sort_time(df,eds_cols, window)
    # sort the pre/post, EDS number
    eds_cols = testing_sort_labels(df,eds_cols, window)
    # sort all the numerical data
    for x in cols_list:
        eds_cols = testing_sort_data(df,x, eds_cols, window)
    # create new dataframe
    eds_df = pd.DataFrame(eds_cols)
    return eds_df
...
class EDS:
    ...
    def get_table(self):
        ...
        if avg_check:
            ...
            if self.mode == 'manual_data.csv':
                man_df = get_avg_manual_data(self.root.filename, self.manual_cols_list, int(self.window))
                output = self.output_loc+"/manual_sorted.csv"
                x = man_df.to_csv(output)
                # upload csv to the table
                self.table.importCSV(output)
            elif self.mode == "noon_data.csv":
                man_df = get_avg_noon_data(self.root.filename, self.noon_cols_list, int(self.window))
                output = self.output_loc+"/noon_sorted.csv"
                x = man_df.to_csv(output)
                # upload csv to the table
                self.table.importCSV(output)
            elif self.mode == "testing_data.csv":
                man_df = get_avg_testing_data(self.root.filename, self.testing_cols_list, int(self.window))
